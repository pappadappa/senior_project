...
dfp_train1, dfp_test1, dft_train1, dft_test1 = train_test_split(df_p1, df_t1, test_size=0.6, random_state=None)
dfp_train2, dfp_test2, dft_train2, dft_test2 = train_test_split(df_p2, df_t2, test_size=0.6, random_state=None)
...
dfp_val1, dfp_test1, dft_val1, dft_test1 = train_test_split(dfp_test1, dft_test1, test_size=0.5, random_state=None)
dfp_val2, dfp_test2, dft_val2, dft_test2 = train_test_split(dfp_test2, dft_test2, test_size=0.5, random_state=None)


dfp_train = pd.concat([dfp_train1,dfp_train2])
dft_train = pd.concat([dft_train1,dft_train2]) 
dfp_val = pd.concat([dfp_val1,dfp_val2]) 
dft_val = pd.concat([dft_val1,dft_val2]) 
...
df_train = dfp_train
dfp_val['result'] = pd.Series(dft_val[0], index=dfp_val.index)
df_val = dfp_val
...
df_val = df_val.sample(frac=1, replace=False, random_state=1)
...
Y_val = np.array(df_val['result'].values)
Y_val = np.reshape(Y_val, (Y_val.shape[0],))
X_val = np.array(df_val[list(range(16384))].values).astype('float32')
X_val = np.reshape(X_val, (X_val.shape[0], 16384, 1))
...
pickle.dump([Y, X, Y_val, X_val, Y_test, X_test], f)
f.close()
